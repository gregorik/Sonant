...
struct FSonantSurfaceDef {
    ...
    UPROPERTY(EditAnywhere, Category = "Sonant")
    TMap<FGameplayTag, FSonantSound> Sounds;
};
...
USTRUCT(BlueprintType)
struct FSonantReverbDef {
    GENERATED_BODY()
    
    UPROPERTY(EditAnywhere, Category = "Sonant")
    TSoftObjectPtr<USoundControlBusMix> ReverbMix;

    UPROPERTY(EditAnywhere, Category = "Sonant")
    float MaxRoomRadius = 1000.0f; 
};
...
public:
    ...
    // 4. Auto Reverb (Smallest matching radius wins)
    UPROPERTY(EditDefaultsOnly, Category = "Reverb")
    TArray<FSonantReverbDef> ReverbSettings;
};